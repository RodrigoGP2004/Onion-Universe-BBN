...
# BBN Evolution (Temperature vs Mass Fraction)
bbn_files = glob.glob("bbn_*.csv")

if not bbn_files:
    print("Warning: No 'bbn_*.csv' files found. Skipping evolution plots.")
else:
    latest_bbn_file = max(bbn_files, key=os.path.getmtime)
    print(f"Reading BBN evolution data from: {latest_bbn_file}")

    df_bbn = pd.read_csv(latest_bbn_file)
    
    T_MeV = df_bbn['T_MeV'].values
    t_num = df_bbn['t'].values

    df_bbn['Heavy'] = 0
    for el, A in heavy_isotopes.items():
        if el in df_bbn.columns:
            df_bbn['Heavy'] += A * df_bbn[el]
    ...
    X_Heavy = df_bbn['Heavy'].iloc[-1]

    print("\n--- Final BBN Mass Fractions ---")
    print(f"Neutrons (n) : {X_n:.6e}")
    print(f"Protons (p)  : {X_p:.6e}")
    print(f"Deuterium (D): {X_d:.6e}")
    print(f"Tritium (T)  : {X_t:.6e}")
    print(f"Helium-3     : {X_3He:.6e}")
    print(f"Helium-4     : {X_4He:.6e}")
    print(f"Lithium-7    : {X_7Li:.6e}")
    print(f"Beryllium-7  : {X_7Be:.6e}")
    print(f"Boron-8      : {X_8B:.6e}")
    print(f"Heavy (A>=9) : {X_Heavy:.6e}")
    ...
    plt.loglog(T_MeV, df_bbn['Heavy'], label=r'A $\geq$ 9', color='purple', linestyle='-', linewidth=lw) 
    ...
    base_name = os.path.splitext(os.path.basename(latest_bbn_file))[0]
    pdf_filename = f"plot_{base_name}.pdf"
...
# Parameter Sweep (Eta) & Likelihood Analysis
sweep_files = glob.glob("abundances_sweep_*.csv")

if not sweep_files:
    print("Warning: No 'abundances_sweep_*.csv' files found. Skipping eta sweep plots.")
else:
    latest_sweep_file = max(sweep_files, key=os.path.getmtime)
    print(f"\nReading parameter sweep data from: {latest_sweep_file}")

    df_eta = pd.read_csv(latest_sweep_file)
    ...
    for el, A in heavy_isotopes.items():
        if el in df_eta.columns:
            df_eta['Heavy'] += A * df_eta[el]
            
    # Observational Constraints (PDG)
    ...
    log_eta_start = np.min(log_eta)
    log_eta_end = np.max(log_eta)
    
    def find_optimal_eta(target):
        val_min = f_D(log_eta_start)
        val_max = f_D(log_eta_end)
        
        if (val_min - target) * (val_max - target) <= 0:
            return brentq(lambda x: f_D(x) - target, log_eta_start, log_eta_end)
        else:
            res = minimize(lambda x: np.abs(f_D(x[0]) - target), 
                           x0=[(log_eta_start + log_eta_end)/2], 
                           bounds=[(log_eta_start, log_eta_end)])
            return res.x[0]

    log_eta_opt = find_optimal_eta(D_obs)
    log_eta_min_val = find_optimal_eta(D_obs - n_sigma * sigma_D)
    log_eta_max_val = find_optimal_eta(D_obs + n_sigma * sigma_D)
    ...
    print("\n--- Optimal Eta from D/H Constraint ---")
    print(f"Optimal eta: {eta_opt:.4e} +/- {delta_eta_avg_2sigma:.4e} (2 sigma)")
    print(f"[2-sigma range: {eta_min_val:.4e} to {eta_max_val:.4e}]")
    print("\nResulting mass fractions at optimal eta:")
    print(f"  p (H) : {f_pr(np.log10(eta_opt)):.5e}")
    print(f"  Y_p   : {f_Yp(np.log10(eta_opt)):.5e}")
    ...
    if np.max(lik_Li) > validity_threshold: 
        lik_Li /= np.max(lik_Li)
    else: 
        lik_Li[:] = 0.0
    ...
    base_name_eta = os.path.splitext(os.path.basename(latest_sweep_file))[0]

    # FIGURE: Final Mass Fractions vs Eta
    ...
    plt.loglog(eta, df_eta['Heavy'], label=r'A $\geq$ 9', color='purple', linestyle='-', linewidth=lw) 
    ...
    file_eta = f"eta_sweep_{base_name_eta}.pdf"
    plt.savefig(file_eta, format='pdf', bbox_inches='tight')
    print(f"Plot saved: {file_eta}")
    ...
    ax2.yaxis.set_label_coords(1.05, 0.3)  
    ...
    file_eta_schramm = f"schramm_plot_{base_name_eta}.pdf"
    plt.savefig(file_eta_schramm, format='pdf', bbox_inches='tight')
    print(f"Plot saved: {file_eta_schramm}")
    ...
    def log_minor_formatter_lik(x, pos):
        if x in [4, 5, 6, 7]:
            return f"{int(x)}"
        return ""

    plt.gca().xaxis.set_major_formatter(ticker.FormatStrFormatter('%g'))
    plt.gca().xaxis.set_minor_formatter(ticker.FuncFormatter(log_minor_formatter_lik))
    
    handles, labels = plt.gca().get_legend_handles_labels()
    plt.legend(handles, labels, loc='center left', bbox_to_anchor=(1.02, 0.5), fontsize=18, ncol=1)
    plt.tight_layout()

    file_eta_lik = f"likelihood_{base_name_eta}.pdf"
    plt.savefig(file_eta_lik, format='pdf', bbox_inches='tight')
    print(f"Plot saved: {file_eta_lik}\n")
